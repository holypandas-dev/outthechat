              ...
              <div>
                ...
                <ul className="space-y-1">
                  {data.what_not_to_bring.map((item, i) => (
                    <li key={i} className="text-sm flex gap-2" style={{ color: 'var(--text-secondary)' }}>
                      <span className="flex-shrink-0" style={{ color: 'var(--accent)', opacity: 0.5 }}>✕</span>
                      {item}
                    </li>
                  ))}
                </ul>
                      ...
                      <ul className="space-y-1">
                        {data.packing_checklist[key].map((item, i) => {
                          ...
                          return (
                            <li key={i}>
                              <label
                                className="flex gap-1.5 items-start cursor-pointer select-none"
                                style={{ color: isChecked ? 'var(--text-muted)' : 'var(--text-secondary)' }}
                              >
                                <span
                                  onClick={() => toggleItem(id)}
                                  className="mt-0.5 flex-shrink-0 w-3 h-3 rounded flex items-center justify-center transition-colors"
                                  style={{
                                    border: `0.5px solid ${isChecked ? 'var(--accent)' : 'var(--border)'}`,
                                ...
                                <span
                                  className="text-xs"
                                  style={{ textDecoration: isChecked ? 'line-through' : 'none', transition: 'all 0.15s' }}
                                  onClick={() => toggleItem(id)}
                                >
                                  {item}
                                </span>
